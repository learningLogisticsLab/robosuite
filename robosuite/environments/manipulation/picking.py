...
class Picking(SingleArmEnv, Serializable):
    ...
    def step(self, action):
        ...
        # If ( OR (ii) we have succeeded, set to true OR (iii) end-effector moves outside the workspace
               ...
               (info['is_success'] and (self.object_names+self.not_yet_considered_object_names) == [])                 # 2. Succeeded AND no more objects. important for multiple object settings when we are done after all objects picked up.
